...
def cart_view(request):
    quantity_range = list(range(1, 11))  # Example range from 1 to 10
...
def update_checkout(request, product_id):
    ...
    response = render(request, 'a_stripe/partials/checkout_total.html', {'product': product_details})
    response['HX-Trigger'] = 'hx_menu_cart'
    return response
...
def checkout_view(request):
    ...
    if shipping_info:
        ...
        form = ShippingInfoForm(initial={'email':request.user.email})
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            print(f'Shipping Info: {shipping_info.email}')
            cart = Cart(request)
...
def payment_cancelled(request):
    return render(request, 'a_stripe/payment_cancelled.html', {'message': 'Payment cancelled! Please try again.'})
...
def stripe_webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        checkout = CheckoutSession.objects.filter(checkout_id=checkout_session_id)
        checkout.has_paid = True
        checkout.save()
        print(f'Payment successful for session: {checkout_session_id}')
